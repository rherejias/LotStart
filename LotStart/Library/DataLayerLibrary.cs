...
namespace LotStart.Library
{
    public class DataLayerLibrary
    {
        ...
        public partial class DataLayer
        {
            ...
            /// </summary>
            /// <param name="strQuery">query for the table</param>
            /// <param name="command">if text or stored procedure</param>
            /// <returns>json string</returns>
            /// author rherejias 4/17/2017
            public string reader(string strQuery, CommandType command)
            {
                string json = "";
                string myCon = ConfigurationManager.AppSettings["oracleConn"];
                try
                {
                    using (OracleConnection cn = new OracleConnection(myCon))
                    {
                        using (OracleCommand cmd = new OracleCommand(strQuery, cn))
                        {
                            ...
                            try
                            {
                                ...
                                int colCount = read.FieldCount;
                                while (read.Read())
                                {
                                    json += "{";
                                    for (int i = 0; i < colCount; i++)
                                    {
                                        string identifier = read.GetName(i);
                                        if (i == colCount - 1)
                                            json += "\"" + identifier + "\"" + ":" + "\"" + read[i] + "\"";
                                        else
                                            json += "\"" + identifier + "\"" + ":" + "\"" + read[i] + "\"" + ",";
                                    }
                                    json += "},";
            ...
            /// <param name="command">sotred proc or text</param>
            /// <returns>json</returns>
            /// rherejias 5/9/2017
            public string local_DB_reader(string strQuery, CommandType command)
            {
                string json = "";
                try
                {
            ...
            /// <returns>json</returns>
            /// rherejias 5/9/2017
            public string local_DB_reader(string strQuery, SqlParameter params_, CommandType command)
            {
                string json = "";
                try
                {
