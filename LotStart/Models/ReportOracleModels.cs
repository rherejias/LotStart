...
{
    public class ReportOracleModels
    {
        ...
        public string getRecord(string moveOrder, string item, string planner, string status, string requiredFrom, string requiredTo, string createdFrom, string createdTo, string pkg, string fromSearch)
        {
            string statusVal = "";
            string[] statusArray = status.Split(',');
            if (status == "" || status == null)
            {
                statusVal = "and mol.line_status in (1,3,5)";
            }
            else
            {
                for (int i = 0; i < statusArray.Length; i++)
                {
                    statusVal += "mol.line_status = '" + statusArray[i] + "' OR ";
                }
                statusVal = statusVal.Remove(statusVal.Length - 3);
                statusVal = "and (" + statusVal + ")";
            }
            ...
            return Library.ConnectionString.returnCon.reader(
                ...
                "sum(mol.quantity_detailed) AllocatedQty, " +
